# For parents

*This is a stub — a fuller version is coming.*

**From Scratch Programming Class** is a free class for kids and teens
(ages 9 and up). The full curriculum is about 150 hours of class
time, which takes roughly one to two years depending on how often
the class meets.

## What your child will learn

The class moves through programming roughly in this order:

1. **Scratch** — visual, drag-and-drop programming. Builds the
   thinking habits before any typing is required.
2. **Python** — a real programming language used in industry.
   Students write, run, and debug their own programs.
3. **Desktop apps and 2D games** — projects students can run on
   their own computers and show off.
4. **Web pages and small web applications** — building things
   that work in a browser.

The full curriculum design and reasoning lives in the
[curriculum decisions document](https://github.com/eric-patton/from-scratch/blob/main/CURRICULUM-DECISIONS.md)
in the project's source repository.

## What it costs

Free. Many local instances of From Scratch also provide computers
that students keep — check with your local instructor.

## When and where

Local logistics — meeting day, time, and venue — vary by class.
Check with your local instructor for specifics.

## Questions

For questions about your local class (logistics, scheduling, the
teacher), contact the class directly.

For questions about the curriculum content (what's being taught,
how, why), the [curriculum decisions document](https://github.com/eric-patton/from-scratch/blob/main/CURRICULUM-DECISIONS.md)
covers most of it.
